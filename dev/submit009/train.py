...
submit009_dir = os.path.abspath(os.path.dirname(__file__))
...
from dev.submit009.model import TransformerAlphaZeroNet
from dev.submit009.state_encoder import StateEncoder
from dev.submit009.mcts import AlphaZeroMCTS


def main():
    submit009_deck = os.path.join(submit009_dir, "deck.csv")
    checkpoint_dir = os.path.join(submit009_dir, "checkpoints")

    config = TrainerConfig(
        experiment_name="submit009_paojian_water_killer_10000ep",
        resume_checkpoint_path=None,

        # Opponent Mix (40% official_sample / 30% sampleAgent001 / 20% self_play / 10% random)
        opponent_types=["official_sample", "sampleAgent001", "self_play"],
        opponent_weights=[0.5, 0.3, 0.2],

        # パオジアンex 超高速水ビートダウンデッキ
        deck_pool_paths=[submit009_deck],
        p1_deck_mode="fixed",
        p2_deck_mode="fixed",
    ...
    print("=" * 80)
    print(f"🚀 Starting Water Killer Training: {config.experiment_name}")
