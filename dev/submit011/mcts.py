...
class AlphaZeroMCTS:
    """Prioritized AlphaZero MCTS Search Engine using Official Search API (search_begin, search_step, search_end)."""
    ...
    def __init__(
        ...
        min_p_threshold: float = 0.02,
        max_pruned_candidates: int = 6
    ):
        self.model = model
    ...
    def _filter_candidate_actions(self, priors: np.ndarray) -> List[int]:
        """Prioritized Action Pruning: Filters out low-probability actions to focus simulation on top candidates."""
        ...
        for idx in sorted_indices:
            ...
            cum_prob += priors[idx]
            if len(candidates) >= self.max_pruned_candidates or cum_prob >= 0.92 or priors[idx] < self.min_p_threshold:
                break

        if len(candidates) == 0:
            candidates = [int(sorted_indices[0])]
        return candidates
    ...
    def get_action_distribution(
        ...
        """Runs Prioritized MCTS search with Search API integration and returns action distribution."""
        ...
        # Apply Dirichlet noise to root priors if dirichlet_epsilon > 0
        ...
        root = MCTSNode()
        candidate_indices = self._filter_candidate_actions(priors)
        ...
        for _ in range(self.num_simulations):
            ...
            if not sim_done and search_obs and search_obs.select and search_obs.select.option:
                ...
                leaf_candidates = self._filter_candidate_actions(leaf_priors)
