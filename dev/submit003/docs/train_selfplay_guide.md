...
2. **GPU重み更新 (Combined Loss Backpropagation)**: 収集した対戦軌跡データを RTX 2070 SUPER（`cuda`）に送り、Value Loss (MSE) ＋ Policy Loss (Cross-Entropy) の合成損失関数で `AlphaZeroNet`（Policy + Value Dual-Head ネットワーク）の重みを更新。
