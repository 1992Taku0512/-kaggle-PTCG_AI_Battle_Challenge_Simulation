...
if os.path.exists(weights_path):
    try:
        ...
        print("Loaded submit011 Prioritized MCTS model weights successfully.")
    except Exception as e:
        print(f"Warning: Failed to load model weights: {e}")

mcts_engine = AlphaZeroMCTS(model=model, encoder=encoder, num_simulations=100, device=device)
...
def agent(obs_dict: dict, context: dict = None) -> list[int]:
    ...
    try:
        action_list, _, _ = mcts_engine.get_action_distribution(obs, your_deck=my_deck, dirichlet_epsilon=0.0)
        if isinstance(action_list, list):
            valid_actions = [a for a in action_list if 0 <= a < num_opts]
            if len(valid_actions) >= min_cnt:
                return valid_actions[:max_cnt]
    except Exception as e:
        pass
